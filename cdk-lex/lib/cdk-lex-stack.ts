...
export class CdkLexStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    super(scope, id, props);

    // s3 
    const s3Bucket = new s3.Bucket(this, "lex-storage",{
    ...
    // API Gateway
    const api = new apiGateway.RestApi(this, 'api-lex', {
      description: 'API Gateway for lex',
    ...
    // POST method
    const lex = api.root.addResource('lex');
    lex.addMethod('POST', new apiGateway.LambdaIntegration(lambdaLex, {
    ...
    distribution.addBehavior("/lex", new origins.RestApiOrigin(api), {
      cachePolicy: cloudFront.CachePolicy.CACHING_DISABLED,
      allowedMethods: cloudFront.AllowedMethods.ALLOW_ALL,  
      viewerProtocolPolicy: cloudFront.ViewerProtocolPolicy.REDIRECT_TO_HTTPS,
    });    
    ...
    new cdk.CfnOutput(this, 'WebUrl', {
      value: 'https://'+distribution.domainName+'/lex.html',      
      description: 'The web url of request for lex',
    });
